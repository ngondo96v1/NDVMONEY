...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    socket.on('admin_notification', (data: { type: string, message: string }) => {
      ...
      if (user?.isAdmin) {
        ...
        if (data.type === 'RANK_UPGRADE') description = 'Yêu cầu nâng hạng thành viên';
        if (data.type === 'NEW_LOAN') description = 'Yêu cầu khoản vay mới';
        if (data.type === 'PAYMENT') description = 'Thông báo thanh toán PayOS';
        
        toast.info(data.message, {
          description
        });
        // Force a data refresh to ensure everything is in sync
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          addNotification(newUser.id, 'Chào mừng thành viên mới', `Chào mừng ${newUser.fullName} đến với hệ thống NDV Money! Tài khoản của bạn đã được khởi tạo với hạn mức ban đầu là ${initialLimit.toLocaleString()} đ. Chúc bạn có những trải nghiệm tuyệt vời cùng chúng tôi.`, 'SYSTEM');
  ...
  const handleApplyLoan = async (amount: number, signature?: string, loanPurpose?: string) => {
    ...
    try {
      ...
      addNotification(user.id, 'Đăng ký vay thành công', `Hợp đồng vay ${contractId} với số tiền ${amount.toLocaleString()} đ đã được gửi thành công. Vui lòng chờ hệ thống xét duyệt.`, 'LOAN');
    ...
    try {
      ...
      if (loanUser) {
        ...
        if (action === 'SETTLE') {
          ...
          const newRankProgress = Math.min(maxOnTimePayments, loanUser.rankProgress + 1);
      ...
      // Notifications
      if (action === 'APPROVE') {
        addNotification(loan.userId, 'Hồ sơ đã được phê duyệt', `Chúc mừng! Hồ sơ vay mã số ${loan.id} của bạn đã được phê duyệt. Vui lòng kiểm tra và thực hiện các bước tiếp theo để nhận giải ngân.`, 'LOAN');
        ...
        if (loan.settlementType === 'PRINCIPAL') {
          addNotification(loan.userId, 'Gia hạn thành công', `Khoản vay mã số ${loan.id} đã được gia hạn thành công. Tổng tiền thanh toán: ${totalPaid.toLocaleString()} đ${fineText}. Thời hạn thanh toán mới của bạn là ngày ${newDueDate}.`, 'LOAN');
        } else if (loan.settlementType === 'PARTIAL') {
          addNotification(loan.userId, 'Tất toán một phần thành công', `Khoản vay mã số ${loan.id} đã được thanh toán một phần gốc với số tiền ${(loan.partialAmount || 0).toLocaleString()} đ. Tổng thanh toán: ${totalPaid.toLocaleString()} đ${fineText}. Dư nợ của bạn đã được cập nhật.`, 'LOAN');
        } else {
          addNotification(loan.userId, 'Tất toán thành công', `Chúc mừng! Khoản vay mã số ${loan.id} của bạn đã được tất toán toàn bộ. Tổng thanh toán: ${totalPaid.toLocaleString()} đ${fineText}. Cảm ơn bạn đã tin dùng dịch vụ của chúng tôi.`, 'LOAN');
        }
  ...
  const handleAdminEditUser = async (userId: string, updatedData: Partial<User>) => {
    ...
    try {
      ...
      addNotification(userId, 'Cập nhật tài khoản', detail, 'SYSTEM');
    } catch (e: any) {
  ...
  const handleAdminEditLoan = async (loanId: string, updatedData: Partial<LoanRecord>) => {
    ...
    try {
      ...
      addNotification(targetLoan.userId, 'Cập nhật khoản vay', detail, 'LOAN');
    } catch (e: any) {
